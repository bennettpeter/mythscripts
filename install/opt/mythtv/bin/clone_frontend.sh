#!/bin/bash
# Clone a frontend or just the keydefs
# 
set -e
...
if [[ "$clonefrom" == "" || "$clonefrom" == "" || "" == "$action" ]] ; then
    echo Usage
    echo "$0 clonefrom cloneto action(I/U/E)"
...
# see if clonefrom is there
count=`echo "select count(*) from settings where hostname = \'$clonefrom\';" | \
  $mysqlcmd | tail -1`
